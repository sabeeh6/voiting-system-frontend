...
export default function VoterElections() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            if (votesRes.data.success) {
                setUserVotes(votesRes.data.data);
            }
        } catch (error) {
    ...
    const hasVotedForElection = (electionId) => {
        return userVotes.some(vote => vote.election === electionId);
    };
            ...
            {isLoading ? (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    {filteredElections.map((election) => {
                        const userAlreadyVoted = hasVotedForElection(election._id);
                        ...
                        return (
                            <motion.div 
                                ...
                                className={`group bg-white rounded-[56px] p-10 border-2 transition-all duration-500 relative flex flex-col ${
                                    userAlreadyVoted ? "border-emerald-100 bg-emerald-50/10 shadow-lg" : "border-slate-50 shadow-xl hover:shadow-2xl hover:border-emerald-100"
                                }`}
                            >
                                <div className="flex justify-between items-start mb-8">
                                    <div className="flex items-center gap-2">
                                        ...
                                        {userAlreadyVoted && (
                                            <div className="px-4 py-1.5 bg-emerald-600 text-white rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 shadow-lg shadow-emerald-600/20">
                                                <ShieldCheck className="w-3 h-3" /> Voted
                                            </div>
                                        )}
                                    </div>
                                        ...
                                        userAlreadyVoted ? (
                                            <div className="flex items-center gap-3">
                                                <button 
                                                    ...
                                                    className="px-6 py-4 bg-slate-100 text-slate-400 rounded-[24px] font-black text-sm flex items-center gap-2"
                                                >
                                                    View Choice
                                                </button>
                                                <div className="px-8 py-4 bg-emerald-50 text-emerald-600 rounded-[24px] font-black text-sm border-2 border-emerald-100 opacity-50 cursor-not-allowed">
                                                    Submitted
                                                </div>
                                            </div>
                                        ) : (
